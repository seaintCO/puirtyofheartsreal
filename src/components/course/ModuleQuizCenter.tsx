    ...
    <section>
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-4">
                ...
                {selected ? (
                  <CheckCircle2 className="text-[#ff75b8]" size={21} />
                ) : (
                  <Circle className="text-white/[0.20]" size={21} />
                )}
              </div>

              <p className="mt-3 text-xs leading-relaxed text-white/[0.40]">
      ...
      <div className="grid gap-6 xl:grid-cols-[260px_minmax(0,1fr)]">
        <aside className="liquid-card-dark rounded-[2rem] p-5">
                ...
                <button
                  ...
                  className={`flex aspect-square items-center justify-center rounded-xl text-xs font-semibold transition ${
                    active
                      ...
                      : answered
                        ? "bg-white text-[#111]"
                        : "bg-white/[0.05] text-white/[0.40] hover:bg-white/[0.1]"
                  }`}
                >
                  {index + 1}
                </button>
          ...
          <div className="mt-7 border-t border-white/[0.10] pt-5">
            <div className="flex justify-between text-xs text-white/[0.40]">
              <span>Answered</span>
              <span>
            ...
            <div className="mt-3 h-2 overflow-hidden rounded-full bg-white/[0.10]">
              <div
                className="h-full rounded-full bg-gradient-to-r from-[#f653a6] to-[#8f75ff] transition-all"
                style={{
                  width: `${
                    (answeredCount /
          ...
          <div className="mt-7 liquid-subcard-dark rounded-2xl p-4">
            <Lock className="text-[#ff75b8]" size={18} />

            <p className="mt-3 text-sm font-medium">
              Passing Requirement
            </p>

            <p className="mt-2 text-xs leading-relaxed text-white/[0.40]">
        ...
        <div className="liquid-card-dark rounded-[2rem] p-5 md:p-8">
          {!submitted ? (
            <>
              <div className="mb-7 flex flex-col justify-between gap-4 border-b border-white/[0.10] pb-6 sm:flex-row sm:items-center">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-widest text-[#ff75b8]">
                    Module {selectedModule.module}
                  </p>
                  ...
                  return (
                    <button
                      ...
                      className={`flex w-full items-start gap-4 rounded-2xl border p-4 text-left text-sm transition md:p-5 ${
                        selected
                          ? "border-[#ff75b8] bg-[#ff75b8]/[0.10] text-white"
                          : "border-white/[0.10] bg-white/[0.045] backdrop-blur-2xl text-white/[0.60] hover:border-white/[0.20] hover:bg-white/[0.06]"
                      }`}
                    >
                      <span
              ...
              <div className="mt-8 flex flex-col justify-between gap-4 sm:flex-row">
                <button
                  ...
                  className="flex items-center justify-center gap-2 rounded-full border border-white/[0.10] px-6 py-3 text-sm text-white/[0.55] transition hover:bg-white/[0.05] disabled:opacity-30"
                >
                  <ArrowLeft size={16} />
                  Previous
                </button>
                ...
                selectedModule.questions.length - 1 ? (
                  <button
                    ...
                    className="flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-[#f653a6] to-[#8f75ff] px-7 py-3 text-sm font-medium text-white"
                  >
                    Next Question
                    <ArrowRight size={16} />
                  </button>
                  ...
                  <button
                    ...
                    onClick={submitQuiz}
                    className="flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-[#f653a6] to-[#8f75ff] px-7 py-3 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-40"
                  >
              ...
              {answeredCount !== selectedModule.questions.length &&
                currentQuestion ===
                  selectedModule.questions.length - 1 && (
                  <p className="mt-4 text-center text-xs text-white/[0.35]">
            ...
            <div>
              <div
                className={`rounded-[2rem] border p-7 text-center ${
                  passed
                    ? "border-green-400/[0.25] bg-green-400/[0.10]"
                    : "border-red-400/[0.25] bg-red-400/[0.10]"
                }`}
              >
                {passed ? (
                  <Trophy
                ...
                <p className="mt-5 text-xs font-semibold uppercase tracking-widest text-white/[0.45]">
                ...
                <p className="mt-2 text-sm text-white/[0.45]">
              ...
              <div className="mt-8 space-y-5">
                {selectedModule.questions.map((item, index) => {
                  ...
                  return (
                    <div
                      key={item.id}
                      className="rounded-2xl border border-white/[0.10] bg-white/[0.045] backdrop-blur-2xl p-5"
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="mt-4 rounded-2xl bg-white/[0.04] p-4 text-xs leading-relaxed text-white/[0.50]">
                            {item.explanation}
                          </p>
                        </div>
                      </div>
